<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,database_connection.*,ToMM.*" errorPage="" %>
...
function getChatData(pid1){ //alert("getChatData called-->"+pid1);
	msg2=pid1;
	var div_inner2="";
	var getStr2=0;
	var strArray2;
...
xmlhttp2.onreadystatechange=function()	
  ...
  if (xmlhttp2.readyState==4 && xmlhttp2.status==200)
	...
	date_time=strArray2[i*5+3];
        U_name="";
	U_name=strArray2[i*5+4];
	//.......
	if(sender==""){div_inner2="";}else{ sender2="";//alert(to_mmStr(sender)); //convert sender to mm
		if(sender==pid1){sender2="<font color=red>"+to_mmStr(U_name)+"</font>";}else{sender2="<font color=green>"+to_mmStr(U_name)+"</font>";}
	div_inner2=div_inner2+sender2+": &nbsp;&nbsp;&nbsp;<font color=brown size=-1>"+to_mmStr(date_time)+"</font><br>&nbsp;&nbsp;"+to_mmStr(message)+"  <br>";} 
	 }
...
function sendChatMsg(){
	msg1=document.getElementById("message2").value;
...
function to_mmStr(pstr){ 
		final_u_name="";//alert(pstr);
	for(i=0;i<pstr.length;i++){ 
		c_char=pstr.charAt(i); 
		c_char=c_char.toLowerCase(); 
		converted_value=eng2mmChar(c_char); //alert("kkk ="+pstr);
		if(converted_value==""){converted_value=c_char;}
	...
	//var key=event.keyCode || event.charCode;	
		...
		c_char=org_values.substr(-1); //alert("("+c_char+")");
			//append this char to the hidden textarea. This value will be saved in the database. starts....
			ta2=pid+"2";
			ta2_values=document.getElementById(ta2).value;
			ta2_values=ta2_values+c_char;
			document.getElementById(ta2).value=ta2_values;
			//append this char to the hidden textarea. This value will be saved in the database. ends....
		//if(key!=8){
		...
		document.getElementById(pid).value=good_part+final_value;
		//}
...
<tr><td><input class="my_button" type="file" name="file1" value="Photo" /><input class="my_button" type="submit" value="&#43972;&#44003;&#43969;&#44000;" /></td></tr></table>
...
String friends_list="";
out.println("<table><tr><td><input type='checkbox' name='all' id='all' checked=true onchange='sel_des_all()'></td><td><font color=red>&#43972;&#44008;&#43971;&#43973;&#44005;&#43971;&#44005;&#43976;</font></td></tr>");
while(rs_fl.next()){
	out.println("<tr><td><input type='checkbox' id='"+rs_fl.getString("member_id")+"' name='"+rs_fl.getString("member_id")+"' checked=true onchange='sel_des_cb(id)'></td><td>"+tmm.convert2MM(rs_fl.getString("Name"))+"</td><td><img src='images/"+rs_fl.getString("p_image")+"' width=30 height=30></td></tr>");
...
<%
		...
		if(request.getParameter("bt_Comment")!=null){
			String comment=request.getParameter("comment2");out.print(comment);//get english value
...
//---------------------------------------------------------------------	
	 out.println ("<div><table><tr><td width='50'><img style='border-radius:5px;' src='images/" +c_p_image +"' width=50 height=50></td><td width='250'>" +tmm.convert2MM(c_p_name) +"<br>"+tmm.convert2MM(rs.getString ("p_date" ))+"</td></tr><tr class='l_name'><td colspan='2'>" +tmm.convert2MM(rs.getString ("description" ))+"</td></tr></table>");
	 if(found==1){
	...
	while(rs04.next()){
	out.println("<table><tr><td width='40'>"+tmm.convert2MM(rs04.getString("Name"))+"<br><img src='images/"+rs04.getString("p_image")+"' width=30 height=30></td><td class='l_name' width='200'>"+tmm.convert2MM(rs04.getString("comment"))+"</td></tr></table>");
	}
	rs04.close (); st04.close ();
	
//--------------------------new comment area starts--------------------------
	 out.println("<form action='' method='post'><table><tr><td><input type='hidden' name='p_id' value='"+rs.getString("sl_no")+"'><textarea id='comment' name='comment' rows='1' cols='30' onkeyup='to_mm(id);'></textarea><textarea id='comment2' name='comment2' rows='1' cols='30' style='display:none'></textarea></td><td><input class='my_button' type='submit' value='&#43971;&#44010;&#43975;' name='bt_Comment' ></td></tr></table></form><hr></div>" );
	}
...
out.println("<table>");
while(rschat.next()){
out.println("<tr><td><img style='border-radius:5px;' src='images/"+rschat.getString("p_image")+"' width=30 height=30></td><td style='max-width:100px;'>"+tmm.convert2MM(rschat.getString("Name"))+"</td><td><input type='button' id='"+rschat.getString("member_id")+"' value='1' style='background-image:url(images/red.png)' onclick='show_hide_messagediv(id)' /></td></tr>");
}	
out.println("</table>");
...
<div id="messagediv" style="display:none; position: absolute; position:fixed; background-color:#CCC; left: 880px; top: 100px; height: 495px; z-index: 10; width: 270px; overflow:scroll; padding-left:2px; border:double; vertical-align:baseline">
    ...
    <div style="position:fixed; top:600px; left: 880px; width: 270px; background-color:#CCC; padding-left:3px; border:double;">
        <textarea name="message" id="message" onkeyup='to_mm(id);'></textarea><textarea id='message2' name='message2' style='display:none'></textarea>
